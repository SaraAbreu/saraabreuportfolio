...
export default function EditCase(){
  ...
  useEffect(()=>{
    try{
      ...
      if (t) setToken(t)
        ...
        const p = typeof window !== 'undefined' && window.prompt && window.prompt('Introduce admin token (si está configurado) o cancelar:')
  ...
  async function load(){
    ...
    if (res.ok) { setContent(data.content); setMsg('Cargado') }
  ...
  async function save(){
    ...
    if (token) headers['x-admin-token'] = token
    const res = await fetch('/api/case/write', { method: 'POST', headers, body: JSON.stringify({ slug, content }) })
    const data = await res.json()
  ...
  return (
    <div className="min-h-screen p-8">
      <Head><title>Admin — Edit Case</title></Head>
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold mb-4">Editar case study</h1>
        <div className="mb-3">
          <select value={slug} onChange={e=>setSlug(e.target.value)} className="p-2 border rounded">
            {projects.map(p=> <option key={p.slug} value={p.slug}>{p.title}</option>)}
          </select>
          <button onClick={load} className="ml-3 px-3 py-2 border rounded">Cargar</button>
        </div>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <div className="flex gap-2 mb-2">
              <button onClick={()=>wrapSelection('**')} className="px-2 py-1 border rounded">B</button>
              <button onClick={()=>wrapSelection('*')} className="px-2 py-1 border rounded italic">I</button>
              ...
              <button onClick={()=>insertAtCursor('\n```\ncode\n```\n')} className="px-2 py-1 border rounded">code</button>
              <button onClick={()=>insertLink()} className="px-2 py-1 border rounded">link</button>
            </div>
            <textarea id="case-editor" value={content} onChange={e=>setContent(e.target.value)} rows={18} className="w-full p-3 border rounded font-mono bg-black/5" />
          </div>
          <div className="p-3 border rounded bg-white/5">
            <h3 className="font-semibold mb-2">Previsualización</h3>
            <div className="prose max-w-none text-gray-100" dangerouslySetInnerHTML={{__html: renderMarkdown(content)}} />
          </div>
        </div>

        <div className="mt-3 flex gap-3">
          <button onClick={save} className="px-4 py-2 bg-sky-500 text-black rounded">Guardar</button>
...
function renderMarkdown(md=''){
  ...
  // code blocks (```)
  out = out.replace(/```([\s\S]*?)```/g, (m, code)=>`<pre class="p-2 bg-black/10 rounded"><code>${escapeHtml(code)}</code></pre>`)
  // headings
