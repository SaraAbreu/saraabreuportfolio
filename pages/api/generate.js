export default async function handler(req, res) {
  try {
    ...
    const { prompt = 'Artwork', style = 'default', count = 6 } = req.body || {}

    // If an external image generation API key is provided (e.g. OpenAI), try to use it.
    const OPENAI_KEY = process.env.OPENAI_API_KEY
    if (OPENAI_KEY) {
      try {
        // Note: This is a minimal scaffold for calling OpenAI Images endpoint.
        // Depending on provider/version the request/response shape may differ.
        const n = Math.max(1, Math.min(12, Number(count) || 6))
        const resp = await fetch('https://api.openai.com/v1/images/generations', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${OPENAI_KEY}` },
          body: JSON.stringify({ prompt: `${prompt} — style: ${style}`, n, size: '1024x1024' })
        })
        const j = await resp.json()
        if (j && j.data && Array.isArray(j.data)){
          const images = j.data.map(d => {
            // some providers return b64_json, some return url — handle both
            if (d.b64_json) return { src: `data:image/png;base64,${d.b64_json}`, thumb: `data:image/png;base64,${d.b64_json}` }
        ...
        // fall back to mock generation below
      }
    }

    // Genera imágenes SVG con variaciones procedurales y thumbnails
    const makeImagePair = (i) => {
      ...
      for (let s = 0; s < shapeCount; s++){
        ...
        if (shapeType === 'circle') shapes += `<circle cx='${sx}' cy='${sy}' r='${Math.max(sw,sh)/2}' fill='hsl(${fillHue} 70% 60%)' opacity='${op}' />`
        else if (shapeType === 'ellipse') shapes += `<ellipse cx='${sx}' cy='${sy}' rx='${sw/2}' ry='${sh/2}' fill='hsl(${fillHue} 70% ${55 - s}%)' opacity='${op}' />`
        else if (shapeType === 'rect') shapes += `<rect x='${sx - sw/2}' y='${sy - sh/2}' width='${sw}' height='${sh}' rx='${20}' fill='hsl(${fillHue} 60% ${50 - s}%)' opacity='${op}' />`
        else shapes += `<polygon points='${sx},${sy} ${sx+sw/2},${sy-sh/3} ${sx-sw/2},${sy+sh/3}' fill='hsl(${fillHue} 70% ${50 - s}%)' opacity='${op}' />`
      }
        ...
        <svg xmlns='http://www.w3.org/2000/svg' width='${w}' height='${h}' viewBox='0 0 ${w} ${h}'>
          <defs>
            ...
            <filter id='noise${i}' x='-50%' y='-50%' width='200%' height='200%'>
              <feTurbulence baseFrequency='0.9' numOctaves='2' seed='${seed % 1000}' stitchTiles='stitch' />
      ...
      // thumbnail: reuse but smaller viewbox and fewer shapes for performance
    ...
    const images = Array.from({ length: Math.max(1, Math.min(12, Number(count) || 6)) }).map((_, i) => makeImagePair(i))
    return res.status(200).json({ images })
  } catch (err) {
